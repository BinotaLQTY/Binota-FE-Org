...
export async function main() {
  ...
  if (networkPreset === "monad") {
    options.name ??= "binota-monad";
    options.network ??= "monad";
  }
  ...
  if (isLocal) {
    await updateNetworksWithLocalUnoToken();
  }
...
async function updateNetworksWithLocalUnoToken() {
  const networksJson = JSON.parse(await fs.readFile(NETWORKS_JSON_PATH, "utf8"));
  const latestDeploymentContext = getLatestDeploymentContext();

  const deployedAddress = latestDeploymentContext?.protocolContracts.UnoToken;
  if (!deployedAddress || (networksJson.local.UnoToken.address === deployedAddress)) {
    return;
  }

  const answer = await question(
    `\nNew UnoToken detected (${deployedAddress}) for local network. Update networks.json? [Y/n] `,
  );
  ...
  networksJson.local.UnoToken.address = deployedAddress;
  await fs.writeFile(NETWORKS_JSON_PATH, JSON.stringify(networksJson, null, 2));

  console.log("");
  console.log("networks.json updated with local UnoToken:", deployedAddress);
}
