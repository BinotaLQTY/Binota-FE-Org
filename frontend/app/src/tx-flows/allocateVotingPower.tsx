...
import { IconDownvote, IconStake, IconUpvote } from "@binota/uikit";
import * as dn from "dnum";
import * as v from "valibot";
import { createRequestSchema, verifyTransaction } from "./shared";

const RequestSchema = createRequestSchema("allocateVotingPower", {
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <Amount value={totalLqtyAllocation} format={2} /> LQTY
                </div>
              </div>
              <div
                className={css({
              ...
              >
                Allocated to {votesCount} initiative
                {votesCount === 1 ? "" : "s"}
              </div>
            </div>
          </div>
      ...
      if (allocations.length === 0) {
        return (
          <TransactionDetailsRow
            ...
            value={["All your votes will be deallocated."]}
          />
        );
      }
      ...
      allocateVotingPower: {
        ...
        async commit(ctx) {
          ...
          const allocationArgs = {
            ...
            votes: Array.from<bigint>({
              length: initiativeAddresses.length,
            }).fill(0n),
            vetos: Array.from<bigint>({
              length: initiativeAddresses.length,
            }).fill(0n),
          };
  ...
  return (
    <TransactionDetailsRow
      label={[
        ...
        <div key="end" title={initiative.address}>
      ...
      value={[
        ...
        >
          {vote.vote === "for" ? "Upvote" : "Downvote"} with{" "}
          <Amount value={lqtyAllocation} /> LQTY
        </div>,
      ]}
    />
  );
