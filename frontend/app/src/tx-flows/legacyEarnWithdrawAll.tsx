...
const RequestSchema = createRequestSchema("legacyEarnWithdrawAll", {
  pools: v.array(
    v.object({
...
export const legacyEarnWithdrawAll: FlowDeclaration<LegacyEarnWithdrawAllRequest> = {
  ...
  async getSteps({ request }) {
    return request.pools.map((pool) => `withdrawFromStabilityPool${pool.branchIndex}`);
  },
...
function getWithdrawStep(legacyBranchIndex: number): Step {
  ...
  return {
    ...
    async commit({ request, writeContract }) {
      const pool = request.pools.find((pool) => pool.branchIndex === legacyBranchIndex);
      if (!pool) {
