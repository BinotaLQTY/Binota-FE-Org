...
const RequestSchema = createRequestSchema("redeemCollateral", {
  amount: vDnum(),
  maxFee: vDnum(),
});
...
export const redeemCollateral: FlowDeclaration<RedeemCollateralRequest> = {
        ...
        {branches.map(({ symbol }) => {
          ...
          return (
            <TransactionDetailsRow
              ...
              value={[
                <Amount key="start" value={collChange} fallback="fetching…" suffix={` ${symbol_}`} />,
                <Fragment key="end">Estimated {symbol_} you will receive.</Fragment>,
              ]}
            />
          );
        })}
  ...
  async getSteps(ctx) {
    ...
    const boldAllowance = await ctx.readContract({
      ...
      args: [ctx.account, getProtocolContract("CollateralRegistry").address],
    });
    ...
    queryFn: async () => {
      ...
      try {
        stored = v.parse(
          ...
          jsonParseWithDnum(localStorage.getItem(`${LOCAL_STORAGE_PREFIX}:simulatedBalancesChange`) ?? ""),
        );
      } catch (_) {
        stored = null;
      }
      ...
      const branches = getBranches();
      const branchesBalanceCalls = branches.map(
        (branch) =>
          ({
