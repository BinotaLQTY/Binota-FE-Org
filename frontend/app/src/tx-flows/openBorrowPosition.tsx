import { ADDRESS_ZERO, InfoTooltip } from "@binota/uikit";
...
export const openBorrowPosition: FlowDeclaration<OpenBorrowPositionRequest> = {
  ...
  Details({ request }) {
    ...
    const yearlyBoldInterest = dn.mul(boldAmount, dn.add(request.annualInterestRate, delegate.data?.fee ?? 0));
        ...
        <>
          ...
          {request.interestRateDelegate ? (
            <TransactionDetailsRow
              ...
              value={[
                <AccountButton key="start" address={request.interestRateDelegate} displayName={delegateDisplayName} />,
                <div key="end">
                  {delegate.isLoading ? (
                    "Loading…"
                  ) : (
                    <>
                      <Amount value={request.annualInterestRate} format="pct2z" percentage />{" "}
                      <Amount
                        percentage
                        format="pct2"
                        prefix="+ "
            ...
            <TransactionDetailsRow
              label="Interest rate"
              value={[
                <Amount key="start" value={request.annualInterestRate} percentage />,
                <Amount
                  key="end"
                  fallback="…"
  ...
  steps: {
    ...
    openTroveLst: {
          ...
          args: [
            {
              ...
              upperHint,
              lowerHint,
              annualInterestRate: ctx.request.interestRateDelegate ? 0n : ctx.request.annualInterestRate[0],
              batchManager: ctx.request.interestRateDelegate ? ctx.request.interestRateDelegate : ADDRESS_ZERO,
      ...
      async verify(ctx, hash) {
        ...
        while (true) {
          const trove = await getIndexedTroveById(branch.branchId, `0x${troveOperation.args._troveId.toString(16)}`);
          if (trove !== null) {
            break;
          }
          await sleep(1000);
