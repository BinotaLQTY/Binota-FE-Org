import { InfoTooltip } from "@binota/uikit";
...
export const updateBorrowPosition: FlowDeclaration<UpdateBorrowPositionRequest> = {
  ...
  Summary({ request }) {
    ...
    const borrowedWithFee = dn.add(loan.borrowed, upfrontFeeData.data?.upfrontFee ?? dn.from(0, 18));
          ...
          <TransactionDetailsRow
            ...
            value={[
              <div
                ...
                style={{
                  color: dn.eq(collChange, 0n) ? "var(--colors-content-alt2)" : undefined,
                }}
              >
...
function getDebtChange(loan: UpdateBorrowPositionRequest["loan"], prevLoan: UpdateBorrowPositionRequest["prevLoan"]) {
  return dn.sub(loan.borrowed, prevLoan.borrowed);
}

function getCollChange(loan: UpdateBorrowPositionRequest["loan"], prevLoan: UpdateBorrowPositionRequest["prevLoan"]) {
...
function useUpfrontFeeData(
  ...
  const upfrontFee = usePredictAdjustTroveUpfrontFee(loan.branchId, loan.troveId, isBorrowing ? debtChange : [0n, 18]);
  ...
  return {
    ...
    data: !upfrontFee.data
      ...
      : {
          isBorrowing,
          debtChangeWithFee: isBorrowing ? dn.add(debtChange, upfrontFee.data) : debtChange,
