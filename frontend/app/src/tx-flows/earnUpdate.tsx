...
const RequestSchema = createRequestSchema("earnUpdate", {
  branchId: vBranchId(),
      ...
      <EarnPositionSummary
        ...
        earnPosition={{
          ...
          deposit: dn.add(
            ...
            request.claimRewards ? DNUM_0 : request.earnPosition.rewards.bold,
          ),
          rewards: {
            // BOLD rewards are claimed or compounded
            bold: DNUM_0,
            coll: request.claimRewards ? DNUM_0 : request.earnPosition.rewards.coll,
          },
        }}
        poolDeposit={request.poolDeposit}
        prevEarnPosition={dn.eq(request.prevEarnPosition.deposit, 0) ? null : request.prevEarnPosition}
      ...
      <>
        ...
        {claimRewards && dn.gt(rewards.coll, 0) && (
          <TransactionDetailsRow
            ...
            value={[
              <Amount key="start" value={rewards.coll} suffix={` ${collateral.name}`} />,
              <Amount key="end" value={collPrice.data && dn.mul(rewards.coll, collPrice.data)} prefix="$" />,
            ]}
          />
        )}
      </>
