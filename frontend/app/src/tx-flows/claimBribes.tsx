...
const RequestSchema = createRequestSchema("claimBribes", {
  initiative: vAddress(),
...
export const claimBribes: FlowDeclaration<ClaimBribesRequest> = {
  ...
  Details({ request }) {
    const { initiativeName, initiative, boldAmount, bribeTokenAmount, bribeTokenAddress, bribeTokenSymbol, claimData } =
      request;
    ...
    return (
      <>
        ...
        >
          Claiming rewards from <span title={initiative}>{initiativeName ?? initiative}</span> over {totalEpochs} epoch
          {totalEpochs > 1 ? "s" : ""}.
        </div>
        ...
        {dn.gt(bribeTokenAmount, 0) && (
          <TransactionDetailsRow
            ...
            value={[
              ...
              >
                <Amount format={2} title={null} value={bribeTokenAmount} />
                <TokenIcon
                  size={16}
                  title={null}
                  token={{
