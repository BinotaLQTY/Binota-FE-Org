...
const RequestSchema = createRequestSchema("legacyRedeemCollateral", {
  amount: vDnum(),
  maxFee: vDnum(),
});
...
export function useSimulatedBalancesChange({
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const branchesBalanceCalls = LEGACY_CHECK.BRANCHES.map(
        (branch) =>
          ({
      ...
      const getBalancesFromSimulated = (position: number) => {
        ...
        return simulation.results.slice(position, position + LEGACY_CHECK.BRANCHES.length + 1).map((result, index) => {
          if (!LEGACY_CHECK) {
            throw new Error("LEGACY_CHECK is not defined");
          }
