import { HFlex, InfoTooltip, StatusDot } from "@binota/uikit";
...
function FooterInfo({ label, value, title }: { label?: ReactNode; value?: ReactNode; title?: string }) {
  return (
    <div
      title={title}
      style={{
    ...
    <FooterInfo
      value={
        ...
        >
          ...
          >
            <StatusDot mode={riskLevelToStatusMode(level)} />
          </div>
          <div
            className={css({
              overflow: "hidden",
...
export const FooterInfoLiquidationRisk = memo(function FooterInfoLiquidationRisk({
  ...
  return (
    <Field.FooterInfoWarnLevel
      help={<InfoTooltip {...infoTooltipProps(content.generalInfotooltips.loanLiquidationRisk)} />}
      label={label}
      level={riskLevel}
    />
  );
    ...
    <Field.FooterInfoWarnLevel
      help={
        <InfoTooltip
          ...
          heading={<FooterInfoRiskLabel label={label} riskLevel={riskLevel} />}
        />
      }
      label={label}
      level={riskLevel}
...
export const FooterInfoLoanToValue = memo(
  function FooterInfoLoanToValue({ ltvRatio, maxLtvRatio }: { ltvRatio: Dnum | null; maxLtvRatio: Dnum }) {
...
export const FooterInfoLiquidationPrice = memo(
  function FooterInfoLiquidationPrice({ liquidationPrice }: { liquidationPrice: Dnum | null }) {
    return (
      <Field.FooterInfo
        label="Liquidation price"
        value={
          <HFlex gap={4}>
            {liquidationPrice ? fmtnum(liquidationPrice, { prefix: "$", preset: "2z" }) : "−"}
            <InfoTooltip {...infoTooltipProps(content.generalInfotooltips.loanLiquidationPrice)} />
          </HFlex>
        }
      />
    );
...
export function FooterInfoRiskLabel({ label, riskLevel }: { label: ReactNode; riskLevel: RiskLevel | null }) {
  return riskLevel ? (
...
export const FooterInfoCollPrice = memo(
  function FooterInfoCollPrice({ collName, collPriceUsd }: { collName: string; collPriceUsd: Dnum }) {
    return (
      <Field.FooterInfo
...
export const FooterInfoMaxLtv = memo(
  function FooterInfoMaxLtv({ maxLtv }: { maxLtv: Dnum }) {
    return (
      <Field.FooterInfo
        label="Max LTV"
        value={
          <HFlex gap={4}>
            <div>{fmtnum(maxLtv, "pct2z")}%</div>
          ...
          <Value negative={dn.gte(props.priceImpact ?? DNUM_0, LEVERAGE_PRICE_IMPACT_HIGH)}>
            <HFlex gap={4}>
              ...
              <InfoTooltip heading="Price impact">
                The difference between the current spot price and the price at which your {props.inputTokenName} gets
                converted to {props.outputTokenName}. Depends on the size of your position and the available liquidity.
              </InfoTooltip>
            </HFlex>
          </Value>
        }
...
export function FooterInfoPriceImpactNone() {
  return <Field.FooterInfo label="Price impact" value="N/A" />;
}

export const FooterInfoSlippageRefundClose = memo(
  function FooterInfoSlippageRefundClose(props: { collateralName: string; slippageProtection: Dnum }) {
    return (
      <Field.FooterInfo
...
export function FooterInfoSlippageRefundNone() {
  return <Field.FooterInfo label="Slippage refund" value="N/A" />;
}
