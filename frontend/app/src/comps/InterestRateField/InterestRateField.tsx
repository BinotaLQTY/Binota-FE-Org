import { Dropdown, InfoTooltip, InputField, Slider, shortenAddress, TextButton } from "@binota/uikit";
...
const DELEGATE_MODES = ["manual", "delegate"] as const;

export type DelegateMode = (typeof DELEGATE_MODES)[number];

const SHOW_AVERAGE_BUTTON_MODES: DelegateMode[] = ["manual"] as const;
...
export const InterestRateField = memo(
  function InterestRateField({
    ...
    const [delegatePicker, setDelegatePicker] = useState<"delegate" | null>(null);
    ...
    }, [averageInterestRate.data, branchId, onAverageInterestRateLoad]);
    ...
    }, [branchId, delegate, onDelegateChange, onModeChange]);
    ...
    const debtInFront =
      loan && interestRate && dn.eq(loan.interestRate, interestRate)
    ...
    const redemptionRisk = debtInFront && getRedemptionRisk(debtInFront.debtInFront, debtInFront.totalDebt);
        ...
        <InputField
          ...
          contextual={match(mode)
            .with("manual", () => (
              <ManualInterestRateSlider
                ...
                handleColor={redemptionRisk && (redemptionRisk === "high" ? 0 : redemptionRisk === "medium" ? 1 : 2)}
              />
            ))
            .with("delegate", () => (
              <TextButton
          ...
          label={{
            start: (
              ...
              <div>
                <Dropdown
                  items={activeDelegateModes.map((mode) => content.interestRateField.delegateModes[mode])}
                  menuWidth={300}
                  menuPlacement="end"
                  onSelect={(index) => {
          ...
          valueUnfocused={
            ...
            mode !== "manual" && delegate === null ? null : <>
              ...
              >
                ...
                >
                  {(mode === "manual" || delegate !== null) && fmtnum(interestRate, "pct2z")}
                </span>
                <span
                  className={css({
                    color: "#878AA4",
