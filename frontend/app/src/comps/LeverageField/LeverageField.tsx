import type { CollateralToken, Drawer } from "@binota/uikit";
import { HFlex, InfoTooltip, InputField, lerp, norm, Slider } from "@binota/uikit";
...
export function useLeverageField({
  ...
  const leverageFactorBeforeAdjustment = dn.toNumber(
    positionDeposit && netDeposit && !dn.eq(netDeposit, DNUM_0) ? dn.div(positionDeposit, netDeposit) : DNUM_1,
  );
  ...
  const quoteData = leverageFactorChange === 0 ? null : (leverageFactorChange > 0 ? quoteLeverUp : quoteLeverDown).data;
  ...
  const actualDebtChange =
    leverageFactorChange === 0
      ? DNUM_0
      : (!quoteData?.bouncing && (leverageFactorChange > 0 ? quoteAmount : quoteAmount && dnumNeg(quoteAmount))) ||
        null;
  ...
  const liquidationPrice =
    (deposit && debt && getLiquidationPrice(deposit, debt, collateralRatio)) ??
  ...
  const {
    ...
    onChange: useCallback(
      ({ focused, parsed }: InputFieldUpdateData) => {
        if (focused && parsed && collPrice) {
          setLeverageFactor(
            getLeverageFactorFromLiquidationPrice(parsed, collPrice, collateralRatio) ?? maxLeverageFactorAllowed,
          );
        }
      },
  ...
  const isValid = !drawer && collPrice && (quoteData === null || quoteData?.bouncing === false);
