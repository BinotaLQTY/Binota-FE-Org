...
export async function getNextOwnerIndex(
  ...
  const { borrowerInfo } = await graphQuery(NextOwnerIndexesByBorrowerQuery, {
...
export async function getIndexedTroveById(
  ...
  return !trove
    ...
    : {
        id: trove.id,
        borrower: (trove.status === "liquidated"
          ? trove.previousOwner
          : trove.borrower) as Address,
        closedAt:
          trove.closedAt === null || trove.closedAt === undefined
            ? null
            : Number(trove.closedAt) * 1000,
...
export async function getAllInterestRateBrackets() {
  try{
    ...
    .map((bracket) => ({
      ...
      pendingDebtTimesOneYearD36: BigInt(0),
...
export async function getGovernanceGlobalData(): Promise<GovernanceGlobalData> {
  const { governanceInitiatives, governanceVotingPower } = await graphQuery(
    GovernanceGlobalDataQuery
  );
  return {
    registeredInitiatives: governanceInitiatives.map(
      (initiative) => initiative.id as Address
    ),
...
export async function getUserAllocationHistoryFromSubgraph(user: Address) {
  const { governanceAllocations } = await graphQuery(
    UserAllocationHistoryQuery,
    {
...
export async function getTotalAllocationHistoryFromSubgraph(
  initiative: Address
) {
  const { governanceAllocations } = await graphQuery(
    TotalAllocationHistoryQuery,
    {
...
// Query for active troves by a specific borrower (for user points calculation)
const ActiveTrovesByBorrowerQuery = graphql(`
  query ActiveTrovesByBorrower($borrower: Bytes!) {
    troves(
...
export async function getActiveTrovesByBorrower(
  borrower: Address
): Promise<TroveForPoints[]> {
  const { troves } = await graphQuery(ActiveTrovesByBorrowerQuery, {
...
// Query for all active troves (for leaderboard)
// Uses pagination to handle large datasets
const AllActiveTrovesQuery = graphql(`
  query AllActiveTroves($skip: Int!, $first: Int!) {
    troves(
      where: { status: active }
      first: $first
      skip: $skip
...
export async function getAllActiveTroves(): Promise<TroveForPoints[]> {
  const allTroves: TroveForPoints[] = [];
  const pageSize = 1000;
  let skip = 0;
  let hasMore = true;
  ...
  while (hasMore) {
    ...
    const mappedTroves = troves.map((trove) => ({
      id: trove.id,
      borrower: trove.borrower as Address,
      debt: dnum18(trove.debt),
      status: trove.status,
    ...
    allTroves.push(...mappedTroves);

    if (troves.length < pageSize) {
      hasMore = false;
    } else {
      skip += pageSize;
    }

    // Safety limit: max 10,000 troves
    if (skip >= 10000) {
      hasMore = false;
    }
  }
...
// Aggregated borrower stats for leaderboard
...
export async function getBorrowerStatsForLeaderboard(): Promise<BorrowerStats[]> {
  const allTroves = await getAllActiveTroves();

  // Aggregate by borrower
  const borrowerMap = new Map<
    string,
    { trovesCount: number; totalDebt: Dnum }
  >();

  for (const trove of allTroves) {
    const borrowerLower = trove.borrower.toLowerCase();
    const existing = borrowerMap.get(borrowerLower);

    if (existing) {
      existing.trovesCount += 1;
      existing.totalDebt = dn.add(existing.totalDebt, trove.debt);
    } else {
      borrowerMap.set(borrowerLower, {
        trovesCount: 1,
  ...
  return Array.from(borrowerMap.entries())
    ...
    .sort((a, b) => dn.cmp(b.totalDebt, a.totalDebt));
}
