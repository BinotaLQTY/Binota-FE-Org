...
import type { Address, CollateralSymbol, CollateralToken } from "@binota/uikit";
...
import { CHAIN_BLOCK_EXPLORER, CHAIN_ID, ENV_BRANCHES, LIQUITY_STATS_URL } from "@/src/env";
...
import { addressesEqual, COLLATERALS, isAddress, shortenAddress, TOKENS_BY_SYMBOL } from "@binota/uikit";
import { useQuery } from "@tanstack/react-query";
import * as dn from "dnum";
import { useMemo } from "react";
import * as v from "valibot";
import { encodeAbiParameters, isAddressEqual, keccak256, parseAbiParameters, zeroAddress } from "viem";
import {  useConfig as useWagmiConfig, useReadContract, useReadContracts, useAccount } from "wagmi";
...
/**
 * Converts a lowercase URL parameter to the correct collateral symbol.
 * URL params are lowercase (e.g., "shmon") but symbols use mixed case (e.g., "shMON").
 */
export function getCollateralSymbolFromParam(param: string): string {
  ...
  return symbolMap[lower] ?? param.toUpperCase();
}
...
export function useStakePosition(address: null | Address) {
  ...
  const BinotaBalance = getProtocolContract("LqtyToken");
  //@TODO: re-introduce governance at some point
  ...
  const userProxyBalance = useReadContract({
    ...BinotaBalance,
  ...
  const stakingCrap = useReadContracts({
    contracts: [{
      ...LqtyStaking,
    ...
    query: {
      enabled: Boolean(address) && accountAddress && userProxyBalance.isSuccess,
      select: ([
        depositResult,
        ...
        const deposit = dnum18(depositResult.result.selfBalance);
        return {
          type: "stake",
          deposit,
          owner: address ?? "0x",
          rewards: dnum18(pendingEthGainResult.result),
        };
      },
    },
  })
