...
import { shortenAddress, Tabs, TokenIcon, VFlex } from "@binota/uikit";
...
{content.stakeScreen.headline}
    ...
    >
      <StakePositionSummary stakePosition={stakePosition.data ?? null} loadingState={stakePosition.status} />
      {bribingClaim.data && bribingClaim.data.claimableInitiatives.length > 0 && (
        ...
        >
          <BribesInfoBox bribingClaim={bribingClaim.data} initiatives={initiatives.data ?? []} />
        </div>
      )}
      <VFlex gap={24}>
        <Tabs
...
function BribesInfoBox({
  ...
  const bribesByInitiative = useMemo(
    () => new Map(claimableInitiatives.map((claim) => [claim.initiative.toLowerCase() as Address, claim])),
    [claimableInitiatives],
  );

  const initiativesByAddress = useMemo(
    () => new Map(initiatives.map((initiative) => [initiative.address.toLowerCase() as Address, initiative])),
    [initiatives],
  );
            ...
            >
              ...
              >
                ...
                {dn.gt(data.bribeTokenAmount, 0) && (
                  ...
                  >
                    <Amount format={2} title={null} value={data.bribeTokenAmount} />
                    <TokenIcon
                      size={16}
                      title={null}
                      token={{
                ...
                >
                  <FlowButton
                    label="Claim"
                    request={() => {
                      const claimData = bribingClaim.claimableInitiatives.find(
                        (c) => c.initiative === initiative,
                      )?.claimData;
                      return (
                        claimData && {
                          flowId: "claimBribes",
