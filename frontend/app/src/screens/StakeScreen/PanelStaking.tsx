...
import { HFlex, InfoTooltip, InputField, Tabs, TextButton, TokenIcon } from "@binota/uikit";
import * as dn from "dnum";
import { useState } from "react";

export function PanelStaking() {
  const account = useAccount();
  const lqtyPrice = usePrice("BINOTA");
  ...
  const lqtyBalance = useBalance(account.address, "BINOTA");
  ...
  const rewardsBinota = stakePosition.data?.rewards ?? DNUM_0;
