import { Checkbox, InfoTooltip, TokenIcon } from "@binota/uikit";
...
import { DNUM_0, dnum18 } from "@/src/dnum-utils";
import { getBranch, getCollToken, isEarnPositionActive } from "@/src/liquity-utils";
import { usePrice } from "@/src/services/Prices";
import type { BranchId, PositionEarn, TokenSymbol } from "@/src/types";
import { useAccount } from "@/src/wagmi-utils";
import { css } from "@/styled-system/css";

export function PanelClaimRewards({ branchId, position }: { branchId: null | BranchId; position?: PositionEarn }) {
  ...
  const gasPriceEth = gasEstimate.data && gasPrice.data ? dnum18(gasEstimate.data * gasPrice.data) : null;

  const txGasPriceUsd = gasPriceEth && ethPrice.data && dn.mul(gasPriceEth, ethPrice.data);
      ...
      >
        ...
        >
          ...
          >
            ...
            <Amount dust={false} format="2z" prefix="~$" value={txGasPriceUsd ?? 0} />
          </div>
        </div>
      </div>
      ...
      >
        {isActive && (
          ...
          >
            ...
            >
              {/** biome-ignore lint/a11y/noLabelWithoutControl: old usage not fxiing this */}
              <label
                className={css({
                  display: "flex",
                  alignItems: "center",
        ...
        <FlowButton
          ...
          request={
            position && {
              ...
              backLink: [`/earn/${collateral.name.toLowerCase()}/claim`, "Back to earn position"],
...
function Rewards({ amount, label, symbol }: { amount: Dnum; label: ReactNode; symbol: TokenSymbol }) {
  return (
    <div
      className={css({
        display: "grid",
