import { Checkbox, HFlex, InfoTooltip, InputField, Tabs, TextButton, TokenIcon } from "@binota/uikit";
...
export function PanelUpdateDeposit({
  ...
  const updatedDeposit = dnumMax(dn.add(position?.deposit ?? DNUM_0, depositDifference), DNUM_0);
  ...
  const withdrawAboveDeposit = mode === "remove" && parsedValue && dn.gt(parsedValue, position?.deposit ?? DNUM_0);

  const allowSubmit =
    account.isConnected && parsedValue && dn.gt(parsedValue, 0) && !insufficientBalance && !withdrawAboveDeposit;
      ...
      >
        {isActive && (
          <HFlex justifyContent="space-between">
            ...
            >
              ...
              >
                <Checkbox id="checkbox-claim-rewards" checked={claimRewards} onChange={setClaimRewards} />
        ...
        <FlowButton
          ...
          request={() => {
            ...
            return {
              flowId: "earnUpdate",
              backLink: [`/earn/${collateral.name.toLowerCase()}`, "Back to editing"],
