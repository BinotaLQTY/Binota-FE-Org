...
import { HFlex, IconEarn, isCollateralSymbol, Tabs } from "@binota/uikit";
...
export function EarnPoolScreen() {
  ...
  const collateralSymbol = getCollateralSymbolFromParam(String(params.pool));
  ...
  const loadingState =
    !ready || earnPool.isLoading || earnPosition.isLoading
      ? "loading"
      : "success";
            ...
            >
              <Tabs
                ...
                onSelect={(index) => {
                  ...
                  router.push(
                    `/earn/${collateralSymbol.toLowerCase()}/${tab.action}`,
                    {
                      scroll: false,
                    }
                  );
