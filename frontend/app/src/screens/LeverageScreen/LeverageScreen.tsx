...
import { DNUM_0, dnum18, dnumMax } from "@/src/dnum-utils";
...
export function LeverageScreen() {
  ...
  const depositUsd = depositPreLeverage.parsed && collPrice.data && dn.mul(depositPreLeverage.parsed, collPrice.data);
  ...
  const allowSubmit = account.isConnected && hasDeposit && leverageField.isValid;
      ...
      <Field
            ...
            contextual={
              <Dropdown
                ...
                onSelect={(index) => {
                  ...
                  router.push(`/multiply/${symbol.toLowerCase()}`, { scroll: false });
                }}
                selected={branch.id}
              />
            }
        ...
        footer={{
          start: collPrice.data && (
            <Field.FooterInfoCollPrice collName={collateral.name} collPriceUsd={collPrice.data} />
          ),
          end: <Field.FooterInfoMaxLtv maxLtv={dn.div(dn.from(1, 18), collateral.collateralRatio)} />,
        }}
      />
