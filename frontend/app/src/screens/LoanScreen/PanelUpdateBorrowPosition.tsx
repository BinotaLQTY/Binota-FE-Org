...
export function PanelUpdateBorrowPosition({ loan }: { loan: PositionLoanCommitted }) {
  ...
  // const boldPriceUsd = usePrice("UNO") ?? { data: dnum18(1000000000000000000) };
  const boldPriceUsd = { data: dnum18(1000000000000000000) };
  ...
  const newDebt =
    debtChange.parsed &&
    (debtMode === "remove" ? dn.sub(loan.borrowed, debtChange.parsed) : dn.add(loan.borrowed, debtChange.parsed));
  ...
  const boldMax = debtMode === "remove" && boldBalance.data ? dnumMin(boldBalance.data, loan.borrowed) : null;
        ...
        <Field
          field={
            <InputField
              ...
              label={{
                start: depositMode === "remove" ? "Decrease collateral" : "Increase collateral",
                end: (
                  <Tabs
                    compact
                    items={[
              ...
              secondary={{
                start: (
                  <Amount value={depositChange.parsed ? dn.mul(depositChange.parsed, collPrice.data) : 0} suffix="$" />
                ),
              ...
              <Field.FooterInfo
                ...
                value={
                  <HFlex alignItems="center" gap={8}>
                    ...
                    >
                      <Amount format={2} suffix={` ${collToken.name}`} value={newLoanDetails.deposit} />
                    </div>
                    <InfoTooltip heading="Collateral update">
                      <div>
                        Before: <Amount format={2} suffix={` ${collToken.name}`} value={loanDetails.deposit} />
                        {" ("}
                        <Amount format={2} prefix="$" value={dn.mul(loanDetails.deposit, collPrice.data)} />
                        {")"}
                      </div>
                      <div>
                        After: <Amount format={2} suffix={` ${collToken.name}`} value={newLoanDetails.deposit} />
                        {" ("}
                        <Amount format={2} prefix="$" value={dn.mul(newLoanDetails.deposit, collPrice.data)} />
                        {")"}
                      </div>
                    </InfoTooltip>
                  </HFlex>
            ...
            <InputField
              ...
              drawer={
                !debtChange.isFocused && isBelowMinDebt
                  ? { mode: "error", message: `You must borrow at least ${fmtnum(MIN_DEBT, 2)} UNO.` }
                  : isAboveMaxLtv
                    ? {
                        mode: "error",
                    ...
                    : insufficientBold
                      ? { mode: "error", message: "Insufficient UNO balance." }
                      : null
              }
              label={{
                start: debtMode === "remove" ? "Decrease loan" : "Increase loan",
                end: (
                  <Tabs
                    compact
                    items={[
              ...
              <Field.FooterInfo
                ...
                value={
                  <HFlex alignItems="center" gap={8}>
                    ...
                    >
                      <Amount value={newLoanDetails.debt} suffix=" UNO" />
                    </div>
                    <InfoTooltip heading="Debt update">
                      <div>
                        Before: <Amount value={loanDetails.debt} suffix=" UNO" />
                      </div>
                      <div>
          ...
          <UpdateBox
            updates={[
              ...
              },
              {
                ...
                after:
                  newLoanDetails.ltv && dn.gt(newLoanDetails.ltv, 0) ? (
                    isAboveMaxLtv ? (
                      <div
                        className={css({
                      ...
                      >
                        <Amount value={maxLtv} prefix=">" percentage />
                      </div>
                    ) : (
        ...
        <WarningBox>
          {loan.batchManager ? (
            <div>
              ...
              below {fmtnum(newLoanDetails.maxLtvAllowed, "pct2z")}%. Please reduce your loan or add more collateral to
              proceed.
            </div>
          ) : (
            <>
              <div>
                Your position's <abbr title="Loan-to-value ratio">LTV</abbr> is {fmtnum(newLoanDetails.ltv, "pct2z")}%,
                which is close to the maximum of {fmtnum(newLoanDetails.maxLtv, "pct2z")}%. You are at high risk of
                liquidation.
              </div>
              <label
