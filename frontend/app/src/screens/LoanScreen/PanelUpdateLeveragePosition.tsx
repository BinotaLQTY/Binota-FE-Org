import { Checkbox, HFlex, InputField, StatusDot, Tabs, TextButton, TokenIcon, VFlex } from "@binota/uikit";
...
export function PanelUpdateLeveragePosition({ loan }: { loan: PositionLoanCommitted }) {
  ...
  const newDepositPreLeverage =
    initialLoanDetails.depositPreLeverage && depositChange.parsed
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: rerender on status change
  useEffect(() => {
    ...
    <>
        ...
        <Field
          field={
            <InputField
              ...
              label={{
                start: depositMode === "remove" ? "Decrease deposit" : "Increase deposit",
                end: (
                  <Tabs
                    compact
                    items={[
              ...
              secondary={{
                start:
                  collPrice.data &&
                  fmtnum(depositChange.parsed ? dn.mul(depositChange.parsed, collPrice.data) : 0, {
                    preset: "2z",
                    prefix: "$",
                  }),
                  ...
                  <HFlex alignItems="center" gap={8}>
                    <ValueUpdate
                      before={
                        <Value
                          negative={
                            initialLoanDetails.depositPreLeverage && dn.lt(initialLoanDetails.depositPreLeverage, 0)
                          }
                          title={`${fmtnum(initialLoanDetails.depositPreLeverage, "full")} ${collToken.name}`}
                        >
          ...
          <UpdateBox
            updates={[
              ...
              },
              {
                ...
                after: <Amount value={newLoanDetails.deposit} suffix={` ${collToken.name}`} />,
              },
              {
                label: "Debt",
                before: (
      ...
      <FlowButton
        ...
        request={{
          ...
          prevLoan: loan,
          depositChange:
            depositChange.parsed && !dn.eq(depositChange.parsed, DNUM_0)
              ? depositMode === "remove"
                ? dnumNeg(depositChange.parsed)
