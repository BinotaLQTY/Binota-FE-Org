import { addressesEqual, Checkbox, HFlex, IconSuggestion, InfoTooltip, StatusDot } from "@binota/uikit";
...
import { fmtnum, formatRelativeTime, formatRisk } from "@/src/formatting";
...
export function PanelInterestRate({
  ...
  const boldInterestPerYear = interestRate && debt.parsed && dn.mul(debt.parsed, interestRate);

  const boldInterestPerYearPrev = loan.interestRate && loan.borrowed && dn.mul(loan.borrowed, loan.interestRate);

  const isDelegated = interestRateMode === "delegate" && interestRateDelegate;
  const allowSubmit =
    Boolean(account.address && addressesEqual(loan.borrower, account.address)) &&
    deposit.parsed &&
    ...
    <>
      <Field
        ...
        footer={{
          start: (
            <Field.FooterInfo
              label={
                updateRateCooldown.status === "success" && (
                  <div
                    className={css({
                      display: "flex",
                      alignItems: "center",
      ...
      <FlowButton
        ...
        request={{
          ...
          backLink: [`/loan/rate?id=${loan.branchId}:${loan.troveId}`, "Back to editing"],
