...
import { addressesEqual, Button, IconExternal, InfoTooltip, Tabs, TokenIcon } from "@binota/uikit";
...
function TroveExplorerLink(props: {
  ...
  return (
    <LinkTextButton
      label={
        <>
          {props.troveExplorer.name} {props.last && <IconExternal size={16} />}
        </>
      }
      href={href}
      external
...
export type LoanLoadingState = "error" | "loading" | "not-found" | "success";
...
export function LoanScreen() {
  ...
  const fullyRedeemed = loan.data && loan.data.status === "redeemed" && dn.eq(loan.data.recordedDebt, 0);
                          ...
                          <>
                            {loan.data.status === "redeemed" && (
                              ...
                              >
                                ...
                                >
                                  {fullyRedeemed ? <>Loan fully redeemed.</> : <>Loan partially redeemed.</>}
                                  <InfoTooltip content={content.generalInfotooltips.redeemedLoan} />
                                </p>

                                <p>
                                  {loan.data.redemptionCount}{" "}
                                  {loan.data.redemptionCount === 1 ? <>redemption</> : <>redemptions</>} since last user
                                  action on{" "}
                                  <time
                            ...
                            <Tabs
                              ...
                              onSelect={(index) => {
                                ...
                                const id = getPrefixedTroveId(loan.data.branchId, loan.data.troveId);
                                router.push(`/loan/${tab.id}?id=${id}`, {
                                  scroll: false,
                                });
                              }}
                            />
...
function ClaimCollateralSurplus({
  ...
  const collSurplusUsd = collPriceUsd.data && collSurplus ? dn.mul(collSurplus, collPriceUsd.data) : null;

  const isOwner = accountAddress && addressesEqual(accountAddress, loan.borrower);
    ...
    >
      ...
      >
        ...
        <div>
          ...
          {isOwner && <>You can claim back the excess collateral accross your liquidated {collToken.name} loans.</>}
        </div>
      </section>
      <Field
        ...
        footer={{
          start: <Field.FooterInfo label={fmtnum(collSurplusUsd, { preset: "2z", prefix: "$" })} value={null} />,
        }}
      />
      {isOwner && (
        <Button
          ...
          onClick={() => {
            if (accountAddress) {
              txFlow.start({
                ...
                backLink: [`/loan?id=${loan.branchId}:${loan.troveId}`, "Back to the loan"],
