import { Button, IconBorrow, IconLeverage } from "@binota/uikit";
...
export const LoadingCard: FC<LoadingCardProps> = ({
  ...
  const title = leverage ? "Multiply" : "UNO loan";
  ...
  const spring = useSpring({
    to: match(loadingState)
      .with(P.union("loading", "error", "not-found"), (s) => ({
        ...
        cardBackground: token("colors.blue:50" as any),
        cardColor: token("colors.blue:950" as any),
      }))
      .otherwise(() => ({
        cardtransform: "scale3d(1, 1, 1)",
        containerHeight: height,
        cardHeight: height,
        cardBackground: token("colors.brandDarkPurple"),
