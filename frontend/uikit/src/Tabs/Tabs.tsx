...
export function Tabs({
  ...
  const [selectedRect, setSelectedRect] = useState<[number, number] | null>(
    null
  );
...
function Tab({
  ...
  const tabColor = compact
    ? selected
      ? token("colors.accentContent")
      : token("colors.interactive")
    : selected
      ? token("colors.accentContent")
      : token("colors.interactive");
  return (
    <button
      ...
      style={{
        color: tabColor,
        padding: compact ? "2px 12px" : "4px 16px",
